...
public class vsbot extends Activity implements View.OnClickListener {
    ...
    public void botplay(){
        ...
        if(level.equalsIgnoreCase("fort")){
            diff = intel_tab_fort(combinaisons(a),p.getListe().get(currentPlayer));
        }
...
/*DEBUT IA*/
    ...
    public static int concatene_nombre(int genere[]) {
    ...
    public static int[] combinaisons(int a) {
        ...
        int[] tab_res = new int[factorielle(a)];
        int[] tab_tmp = new int[a];// tab tempo
        int[] tab = new int[a];
        tab = genere(a); // REMPLI par NB alea
        System.out.println("Nombre obtenus : ");
        affichetab(tab);
        System.out.println();
        ...
        //tab_res[0] = concatene_nombre(tab_tmp);
        int j = 0;
        while (j != factorielle(a)) {

            permut(tab_tmp);
            tab_res[j] = concatene_nombre(tab_tmp);
            j++;
    ...
    public static int intel_tab_fort(int[] tab,Joueur r) {
            ...
            if(reste-tab[i]>=0){
                if (!(reste<1111 && reste >=1000)){
                    if (!(reste<111 && reste >=100)){
                        if(reste!=10){
                                res = tab[i];
                                System.out.println("SA PASSE :" + tab[i]);
